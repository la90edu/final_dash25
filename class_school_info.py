...
class SchoolInfo:
#   def __init__(self,df,anigma_name,anigmas,global_average): 
  def __init__(self,df,anigmas,global_average): 

    
    self.df = df
    self.global_average=global_average
    self.anigmas=anigmas
  ...
  def get_fig_ici(self,name):
      graph=Gauge_Graph_type("ici",name,self.ici,self.global_average)
      fig=graph.get_fig()
      return fig

      
  def get_fig_risc(self,name):
      graph=Gauge_Graph_type("risc",name,self.risc,self.global_average)
      fig=graph.get_fig()
      return fig
  
  def get_fig_spider(self, mobile=False):
      graph=Spider_Graph_type("spider",self.return_anigmas_result_as_dict(),self.global_average)
      if mobile:
  ...
  def get_precentage_diffrent_from_global_anigmas(self,anigma_name):
        global_anigmas=self.global_average#st.session_state.global_average
