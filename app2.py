...
import init
import tab1_hegedim2        
import tab2_hegedim1_2  
def main():
    st.set_page_config(
    ...
    st.write("🔗 פרמטרים שהתקבלו מה-URL:"   
            f" כיתה: {st.session_state.get('class_name', 'לא הועבר')}, "
            f" בית ספר: {st.session_state.get('school_name', 'לא הועבר')}")
    ...
    if not df1.empty and 'school' in df1.columns:
        unique_schools = df1["school"].unique().tolist()
        selected_school = st.selectbox("בחר בית ספר:", unique_schools, key="school_selector")
        filtered_df1 = df1[df1['school'] == selected_school]
        st.session_state.filtered_df1 = filtered_df1
       
    if not df2.empty:
            # unique_schools = df2["school"].unique().tolist()
            # selected_school = st.selectbox("בחר בית ספר:", unique_schools, key="school_selector")
            filtered_df2 = df2[df2['school'] == selected_school] #if selected_school else df2
...
def show_tabs():
        ...
        with tab2:
                tab2_hegedim1_2.show(st.session_state.filtered_df1,st.session_state.global_average1, st.session_state.filtered_df2, st.session_state.global_average2)

        with tab3:
                st.warning("העמוד הזה עדיין בפיתוח. בקרוב יהיו בו הצעות לשיפור.")
        # show_charts_page()
...
def show_data_page():
    ...
    if school_name and class_name:
        st.info(f"השוואה עבור: {school_name} - {class_name}")
        # כאן ניתן להוסיף לוגיקת השוואה
    else:
        st.warning("בחר בית ספר וכיתה כדי לראות השוואות")

def show_charts_page():
    st.header("💡 הצעות לשיפור")
    st.write("המלצות לשיפור על בסיס הנתונים")
    
    school_name = st.session_state.get('school_name', None)
    class_name = st.session_state.get('class_name', None)
    
    if school_name and class_name:
        st.info(f"הצעות שיפור עבור: {school_name} - {class_name}")
        # כאן ניתן להוסיף לוגיקת הצעות שיפור
    else:
        st.warning("בחר בית ספר וכיתה כדי לקבל הצעות שיפור")
